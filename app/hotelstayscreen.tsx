...
export default function HotelStayScreen() {
    ...
    <View style={styles.wrapper}>
            ...
            <View style={styles.headerContent}>
              <View style={[styles.headerRow, isRTL && { flexDirection: 'row-reverse' }]}>
                ...
                <View style={styles.headerTextContainer}>
                  <Text style={[styles.headerTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.yourHotelStay')}</Text>
                </View>
                <TouchableOpacity
      ...
      >
        ...
        <View style={styles.card}>
          <Text style={[styles.cardTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelDetails')}</Text>

          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelName')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelNameValue')}</Text>
          </View>

          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.location')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.locationValue')}</Text>
          </View>
          ...
          <View style={[styles.infoRowSplit, isRTL && { flexDirection: 'row-reverse' }]}>
            <View style={[styles.infoSplitItem, isRTL && { paddingRight: 0, paddingLeft: 12 }]}>
              <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.checkIn')}</Text>
              <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.checkInValue')}</Text>
            </View>
            <View style={styles.infoSplitItem}>
              <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.checkOut')}</Text>
              <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.checkOutValue')}</Text>
            </View>
          </View>

          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.roomNumber')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.roomNumberValue')}</Text>
          </View>

          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.groupAssignedFloor')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.groupAssignedFloorValue')}</Text>
          </View>
        </View>

        {/* Facilities & Amenities Card */}
        <View style={styles.card}>
          <Text style={[styles.cardTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.facilitiesAmenities')}</Text>
          <View style={styles.facilitiesGrid}>
            {[
              t('hotelStay.facilityBeds'),
              t('hotelStay.facilityAirConditioning'),
              t('hotelStay.facilityBreakfast'),
              t('hotelStay.facilityWaterTea'),
              t('hotelStay.facilityWifi'),
              t('hotelStay.facilityBathroom'),
              t('hotelStay.facilityLaundry'),
              t('hotelStay.facilityCharging'),
              t('hotelStay.facilityPrayerArea'),
            ].map((item) => (
        ...
        {/* Hotel Guidelines Card */}
        <View style={styles.card}>
          <Text style={[styles.cardTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelGuidelines')}</Text>

          {[
            t('hotelStay.guideline1'),
            t('hotelStay.guideline2'),
            t('hotelStay.guideline3'),
            t('hotelStay.guideline4'),
          ].map((text, index) => (
            <View key={index} style={[styles.guidelineRow, isRTL && { flexDirection: 'row-reverse' }]}>
              <View style={styles.guidelineBullet} />
              <Text style={[styles.guidelineText, isRTL && { textAlign: 'right' }]}>{text}</Text>
            </View>
          ))}

          <Text style={[styles.cardTitle, styles.subSectionTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.checkInProcess')}</Text>
          <View style={styles.timelineContainer}>
            {[
              t('hotelStay.checkInStep1'),
              t('hotelStay.checkInStep2'),
              t('hotelStay.checkInStep3'),
            ].map((step, index, arr) => (
              <View key={index} style={[styles.timelineRow, isRTL && { flexDirection: 'row-reverse' }]}>
                ...
                <View style={[styles.timelineInfoCol, isRTL && { paddingRight: 8, paddingLeft: 0 }]}>
                  <Text style={[styles.checkinText, isRTL && { textAlign: 'right' }]}>{step}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        <View style={styles.card}>
          <Text style={[styles.cardTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.dailySchedule')}</Text>
          <View style={styles.timelineContainer}>
            {[
              { day: t('hotelStay.scheduleDay1'), text: t('hotelStay.scheduleText1') },
              { day: t('hotelStay.scheduleDay2'), text: t('hotelStay.scheduleText2') },
              { day: t('hotelStay.scheduleDay3'), text: t('hotelStay.scheduleText3') },
        ...
        <View style={styles.card}>
          <Text style={[styles.cardTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.emergencySupport')}</Text>

          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelReception')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelReceptionValue')}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.groupSupervisor')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.groupSupervisorValue')}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, isRTL && { textAlign: 'right' }]}>{t('hotelStay.lostFound')}</Text>
            <Text style={[styles.infoValue, isRTL && { textAlign: 'right' }]}>{t('hotelStay.lostFoundLocation')}</Text>
          </View>
        </View>
      ...
      {/* Sticky Open Hotel Location Button */}
      <View style={styles.stickyButtonContainer}>
        <TouchableOpacity style={styles.stickyButton}>
          ...
          >
            ...
            <Text style={styles.stickyButtonText}>{t('hotelStay.openHotelLocation')}</Text>
          </LinearGradient>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.noticeOverlay}>
          ...
          >
            ...
            <View style={styles.noticeInner}>
              <Text style={[styles.noticeTitle, isRTL && { textAlign: 'right' }]}>{t('hotelStay.hotelStayReminders')}</Text>
              <Text style={[styles.noticeBody, isRTL && { textAlign: 'right' }]}>
                {t('hotelStay.hotelStayRemindersBody')}
              </Text>
              <TouchableOpacity
